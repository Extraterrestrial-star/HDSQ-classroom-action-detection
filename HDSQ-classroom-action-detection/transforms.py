...
def build_transforms(cfg=TransformsCfg(), is_train=True, args=None):
  ...
  # Dual-stream training: keep a longer aligned temporal window (e.g., L=64) and let the model
  # sample slow/fast pathways (16@4 and 32@2) internally.
  if args is not None and getattr(args, "dual_stream", False):
    ...
    frame_num = slow_frames * slow_stride  # default 16*4=64
    sample_rate = 1
